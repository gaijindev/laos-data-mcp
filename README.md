...
- [Requesting Laosis access](#requesting-laosis-access)
...
- pnpm 10+. A top-level `packageManager` field lets `corepack` fetch the pinned pnpm version.
...
default; set `COMTRADE_API_KEY` for the full endpoint.
...
- LSB SDG is official but updates infrequently; treat coverage as periodic, not real-time.
...
settings. Public/no-auth sources work without `.env`, subject to upstream availability.

| Variable                   | Purpose                                                    |
| -------------------------- | ---------------------------------------------------------- |
| `LAOSIS_API_KEY`           | Enables Laosis live API calls where an access path exists. |
...
| `MRC_SESSION_TOKEN`        | MRC portal token for raw-data access.                      |
...
Statistics Bureau. It does not expose a documented public REST API, so the adapter exposes
known categories and activates live calls only when an API/key path is available.
