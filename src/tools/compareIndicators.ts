...
export function registerCompareIndicatorsTool(server: McpServer): void {
  server.registerTool(
    ...
    async ({ indicators, startYear, endYear, outputFormat }) => {
      ...
      // Each spec usually resolves to a *different* upstream, so we fetch them
      // concurrently — latency becomes the slowest single fetch instead of the
      // sum of all of them. allSettled (not all) keeps the per-spec partitioning:
      // one source failing or returning no data still yields the others, exactly
      // as the old sequential try/catch-per-spec did. Order is preserved by
      // mapping over the original `indicators` array.
      const settled = await Promise.allSettled(
        indicators.map(async (spec) => {
          ...
          if (!fetcher) {
            return { kind: "note" as const, note: `${label}: ${unsupportedSourceHint(source)}` };
          }
          const records = await fetcher(code, start, end);
          if (records.length === 0) {
            return {
              kind: "note" as const,
              note: `${label}: no data for ${code} (${source}) in ${start}–${end}.`,
            };
          }
          return {
            kind: "input" as const,
            input: {
              ...
              records,
            } satisfies ComparisonSeriesInput,
          };
        }),
      );
      ...
      settled.forEach((outcome, i) => {
        if (outcome.status === "fulfilled") {
          if (outcome.value.kind === "input") inputs.push(outcome.value.input);
          else notes.push(outcome.value.note);
        } else {
          const spec = indicators[i];
          if (!spec) return; // settled is mapped 1:1 from indicators, so i is always valid
          const label = spec.label ?? spec.code;
          notes.push(`${label}: ${toToolErrorMessage(outcome.reason, { subject: spec.code })}`);
        }
      });
