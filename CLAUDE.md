...
19. UN Global SDG (`unSdg.ts`) — REST JSON, no auth. Area M49 `418`. Food/agriculture, housing/community,
    crime/law/justice. Curated `KEY_UN_SDG_INDICATORS`.
20. FAOLEX (`faolex.ts`) — POST search, no auth. Country filter `country:("LAO")`. Legal texts, laws,
    regulations, policies. Returns `DatasetMetadata`, not `IndicatorRecord`.
21. World Bank Data360 (`data360.ts`) — REST JSON, no auth. ISO3 `LAO`. Governance / WGI rule-of-law
    indicators. Curated `KEY_DATA360_INDICATORS`.
...
fetcher/tool. Existing sources and the record schemas are never modified. **When you add
a source, also (a) add it to this "Active data sources" list, (b) bump the source count in
this file's intro AND in the `INSTRUCTIONS` string in `src/server.ts`, and (c) add a gotcha
below if it has one.** These three spots drift silently otherwise.
...
- **UN Global SDG** uses M49 `418` and **two endpoints**: `/Indicator/Data` when the code matches
  the indicator pattern (e.g. `16.1.1`), else `/Series/Data`. Responses are **paginated** (pageSize
  1000, loop `totalPages`); decode value/unit labels against the `attributes`/`dimensions` metadata
  blocks. Filter rows to `geoAreaCode === "418"`.
- **FAOLEX** is a **POST** to a Google-search-application endpoint with `Content-Type: text/plain`
  (body is JSON carrying `searchApplicationId`). Query is built as `country:("LAO") <typeFilter> <terms>`;
  user input is sanitized (Lucene metachars stripped). PDFs resolve against `faolex.fao.org/docs/pdf`.
- **Data360** uses ISO3 `LAO` and requires `DATABASE_ID` + `INDICATOR` + `COMP_BREAKDOWN_1=WGI_EST`
  params; values carry **`UNIT_MULT` (×10^n)** like ILOSTAT. WGI indicators are on a -2.5..2.5 scale.
